"""Test-only RSA-sleutelpaar voor het simuleren van centrale SureSync ID-tokens.

Uitsluitend voor de testsuite. Dit paar is NIET het productie- of staging-paar en
wordt nergens buiten de tests gebruikt.
"""

PRIV = """-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----"""

PUB = """-----BEGIN PUBLIC KEY-----

JQIDAQAB
-----END PUBLIC KEY-----"""


def central_token(claims: dict) -> str:
    """Onderteken een centraal SSO-token met de testsleutel."""
    from jose import jwt

    return jwt.encode({**claims, "iss": "suresync-id"}, PRIV, algorithm="RS256")
